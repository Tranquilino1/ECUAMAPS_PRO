package app.ecuamaps.util;

import static app.ecuamaps.sdk.util.Utils.dimen;
import static app.ecuamaps.sdk.util.Utils.isIntentSupported;
...
import app.ecuamaps.sdk.util.Distance;
...
import app.ecuamaps.sdk.util.log.LogsManager;
...
public class Utils
{
  ...
  public static void copyTextToClipboard(Context context, String text)
  {
    ...
    final ClipData clip = ClipData.newPlainText("ECUAMAPS: " + text, text);
  ...
  {
    subject = "ECUAMAPS Bugreport" + (TextUtils.isEmpty(subject) ? "" : ": " + subject);
  ...
  {
    LogsManager.INSTANCE.zipLogs(
        new SupportInfoWithLogsCallback(launcher, activity, "ECUAMAPS Feedback", "", BuildConfig.SUPPORT_MAIL));
  }
  ...
  {
    if (MwmApplication.from(context).getecuamaps().arePlatformAndCoreInitialized())
      return;
  ...
  public static String getDonateUrl(@NonNull Context context)
  {
    ...
    // Enable donations by default if not Google or Huawei. Replace ecuamaps.app/donate/ with localized page.
    if ((url.isEmpty() && !BuildConfig.FLAVOR.equals("google") && !BuildConfig.FLAVOR.equals("huawei"))
        || url.endsWith("ecuamaps.app/donate/"))
