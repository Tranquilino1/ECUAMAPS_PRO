...
std::string BuildIndexFile(std::vector<std::string> const & filesForIndex)
{
  ...
  std::string content =
      ...
      "<Document>\n"
      "<name>ECUAMAPS Bookmarks and Tracks</name>\n";
...
{
  auto const kmzFileName = "ecuamapsBackup_" + std::to_string(base::GenerateYYMMDD(time(nullptr)));
  ...
  for (auto const & kmlToExport : *collection)
  {
    ...
    if (!strings::IsASCIIString(fileName))
      fileName = "ecuamaps_" + std::to_string(suffix++);
