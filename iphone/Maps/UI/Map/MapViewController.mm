...
- (void)viewDidLoad
{
  ...
  // Added in https://github.com/ecuamaps/ecuamaps/pull/7333
...
- (void)performAction:(NSString *)action
{
  ...
  if (self.isViewLoaded)
  {
    ...
    if ([action isEqualToString:@"app.ecuamaps.3daction.bookmarks"])
      [self.bookmarksCoordinator open];
    else if ([action isEqualToString:@"app.ecuamaps.3daction.search"])
      [self.searchManager startSearchingWithIsRouting:NO];
    else if ([action isEqualToString:@"app.ecuamaps.3daction.route"])
      [self.controlsManager onRoutePrepare];
    else if ([action isEqualToString:@"app.ecuamaps.3daction.report_bug"])
      [MailComposer sendBugReportWithTitle:@"Bug Report / ECUAMAPS"];
  }
  else
