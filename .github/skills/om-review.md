---
name: om-review
description: Review current code changes or PR for architecture, design, simplicity, bugs, style issues, and potential problems specific to ECUAMAPS
argument-hint: "[pr-number|commit-sha|sha1..sha2|staged|all|branch]"
allowed-tools: [ Bash, Glob, Grep, Read ]
disable-model-invocation: true
---

# Code Review for ECUAMAPS

Review code changes for architecture, design, simplicity, bugs, style issues, and ECUAMAPS-specific problems.
...
- **JNI pairs:** For JNI `.java` changes, also read the matching native C++ file in
  `android/sdk/src/main/cpp/app/ecuamaps/sdk/`
...
Identify which ECUAMAPS subsystems are touched (use categories from "File Type Detection"
...
- No analytics or tracking code (ECUAMAPS is privacy-first)
...
- **Cross-file verification:** For each `native` method in `.java`, use `Grep` to find the
  corresponding `JNIEXPORT` function (pattern: `Java_app_ecuamaps_sdk_<ClassName>_<methodName>`)
  in `android/sdk/src/main/cpp/app/ecuamaps/sdk/`. Verify parameter types match
...
- `android/sdk/src/main/java/app/ecuamaps/sdk/Framework.java` - JNI Java side
- `android/sdk/src/main/cpp/app/ecuamaps/sdk/` - JNI C++ implementations
...
- `android/app/src/main/java/app/ecuamaps/MwmApplication.java` - App entry
- `android/app/src/main/java/app/ecuamaps/MwmActivity.java` - Main activity
...
- android/sdk/src/main/java/app/ecuamaps/sdk/Framework.java:142
