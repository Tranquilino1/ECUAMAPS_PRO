...
`maps_generator` is the Python CLI for generating `.mwm` maps for the ECUAMAPS application. This tool functions as the driver for the `generator_tool` C++ executable.
...
The app version can be found in the "About" section of ECUAMAPS app.
...
5. Create a [configuration file with defaults](https://github.com/ecuamaps/ecuamaps/blob/master/tools/python/maps_generator/var/etc/map_generator.ini.default):
...
If you are not from the ECUAMAPS team, then you do not need the `--production` option.
...
You can manually generate a subway layer file to use in the `SUBWAY_URL` ini setting. See [instructions](https://github.com/ecuamaps/ecuamaps/tree/master/docs/SUBWAY_GENERATION.md).
