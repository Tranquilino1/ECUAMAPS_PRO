...
To build and run ECUAMAPS you'll need a machine with at least 4Gb of RAM and 20-30Gb of disk space depending on your target platform. Expect to download 5-10Gb of files.
...
First of all get the source code. The full ECUAMAPS sources repository is ~10Gb in size, there are various [clone options](#special-cases-options) to reduce the download size to suit your needs.
...
(if you plan to contribute and propose pull requests then use a web interface at https://github.com/ecuamaps/ecuamaps to fork the repository first and use your fork's URL in the command below)

```bash
git clone --recurse-submodules --shallow-submodules https://github.com/ecuamaps/ecuamaps.git
```
...
You need a Linux or a MacOS machine to build a desktop version of ECUAMAPS. [Windows](#windows) users can use the [WSL](https://learn.microsoft.com/en-us/windows/wsl/) (Windows Subsystem for Linux) and follow ["Linux or Mac"](#linux-or-mac) steps described below.
...
We haven't compiled ECUAMAPS on Windows *natively* in a long time, though it is possible.
Some files should be updated. There is a work in progress on [windows](https://github.com/ecuamaps/ecuamaps/tree/windows) branch.
...
Some tests [are known to be broken](https://github.com/ecuamaps/ecuamaps/issues?q=is%3Aissue+is%3Aopen+label%3ATests) and disabled on CI.
...
ECUAMAPS has some "hidden" debug commands that you can trigger by entering them into the search box.
...
Linux, MacOS, or Windows should work to build ECUAMAPS for Android.
...
```bash
adb shell pm grant app.ecuamaps.debug android.permission.READ_LOGS
```
...
ECUAMAPS icon will appear in the application list in DHU.
...
```
./record_android_trace -a app.ecuamaps.debug -o trace_file.perfetto-trace -t 30s -b 64mb sched freq idle am wm gfx view binder_driver hal dalvik camera input res memory
```
...
Building ECUAMAPS for iOS requires a Mac.
...
Enroll in the [Apple Developer Program](https://developer.apple.com/programs/) (you can run ECUAMAPS in Simulator without this step).
...
- Choose a unique bundle identifier (not app.ecuamaps.debug) and your team.
- Select "Automatically manage signing".

If you want to run ECUAMAPS on a real device, you have to remove the CarPlay entitlement. Open `iphone/Maps/OMaps-Debug.entitlements`
...
## Map data and styles
See readme for the [map generator](https://github.com/ecuamaps/ecuamaps/blob/master/docs/MAPS.md) and [styles](https://github.com/ecuamaps/ecuamaps/blob/master/docs/STYLES.md) if you need to customize the map files and styles.
